...
const stopProgress = (success) => {
  ...
  if ((success && progress.value < 100) || (!success && progress.value > 0)) {
    setProgressInterval.value = setInterval(() => {
      if (
        (success && progress.value >= 100) ||
        (!success && progress.value <= 0)
      ) {
...
<template>
  ...
  <div
    v-if="visible"
    class="w-full h-1/2 flex justify-center items-end pb-8 font-semibold text-sm text-white"
  >
    <v-progress-linear
      v-if="type === 'linear'"
